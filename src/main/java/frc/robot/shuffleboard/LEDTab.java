...
public class LEDTab extends ShuffleboardTabBase {
	...
	public void activateShuffleboard() {
		...
		tab.add("Disconnected Animation",
				new InstantCommand(() -> led.setDisconnectedAnimation()).ignoringDisable(true));
		tab.add("EStop Animation", new InstantCommand(() -> led.setEStopAnimation()).ignoringDisable(true));
		tab.add("Auto Disabled Animation",
				new InstantCommand(() -> led.setAutoDisabledAnimation()).ignoringDisable(true));
		tab.add("Auto Enabled Animation",
				new InstantCommand(() -> led.setAutoEnabledAnimation()).ignoringDisable(true));
		tab.add("Teleop Disabled Animation",
				new InstantCommand(() -> led.setTeleopDisabledAnimation()).ignoringDisable(true));
		tab.add("Teleop Enabled Animation",
				new InstantCommand(() -> led.setTeleopEnabledAnimation()).ignoringDisable(true));
		tab.add("Holding Note Animation", new InstantCommand(() -> {
			led.holdingNote = true;
		}).ignoringDisable(true));
		tab.add("Ready To Shoot Animation", new InstantCommand(() -> {
			led.readyToShoot = true;
		}).ignoringDisable(true));
