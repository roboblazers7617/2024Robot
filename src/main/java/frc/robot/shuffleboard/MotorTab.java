...
/**
 * This class is used to create a tab on the shuffleboard that displays
 * information about the motor power and current
 */
public class MotorTab extends ShuffleboardTabBase {
	...
	public void addMotor(CANSparkMax[] newMotors) {
		if (newMotors.length + numberOfMotors > Constants.NUMBER_OF_MOTORS) {
			tooManyMotors.setText(
					"Too many motors. Increase Constants.NUMBER_OF_MOTORS to: " + (newMotors.length + numberOfMotors));
		...
		for (int i = 0; i < newMotors.length; i++) {
			...
			busVoltagePublishers[i + numberOfMotors] = networkTable
					.getDoubleTopic("Motor: " + (i + numberOfMotors) + " Bus Voltage").publish();
			optionCurrentPublishers[i + numberOfMotors] = networkTable
					.getDoubleTopic("Motor: " + (i + numberOfMotors) + " Total Current").publish();
			stickyFaultPublisher[i + numberOfMotors] = networkTable
					.getDoubleTopic("Motor: " + (i + numberOfMotors) + " Sticky Faults").publish();
		}
